...
// GetSwapByStatus ...
func (d *DataBase) GetSwapByStatus(sender, receipt string, amount string) (*Swap, error) {
	swap := &Swap{}
	if err := d.db.Where("sender_addr = ? and receiver_addr = ? and out_amount = ? and status in (?)",
